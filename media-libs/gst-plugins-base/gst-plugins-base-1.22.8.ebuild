...
IUSE="alsa +egl gbm gles2 +introspection ivorbis +ogg +opengl +orc +pango theora +vorbis wayland +zink +X"
GL_REQUIRED_USE="
	|| ( gbm wayland X )
	wayland? ( egl )
	gbm? ( egl )
...
GL_DEPS="
	>=media-libs/mesa-9.0[egl(+)?,gbm(+)?,gles2?,wayland?,zink(+),${MULTILIB_USEDEP}]
	egl? (
		x11-libs/libdrm
	)
	gbm? (
